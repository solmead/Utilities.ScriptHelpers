...
export class GameLayer extends Layer {
    ...
    setupLayer() {
        ...
        for (var i = 0; i < 15; ++i) {
            ...
            var scale = (Math.random() * 16);
            //scale = 1;
            var t = Math.random();
            if (t > 0.75) {
                var crate = new Crate(pos, geometry.sizeMake(16 + scale, 16));
    ...
    mouseDown(evt) {
        var point = evt.locationInCanvas, world = this.pLayer.world, mouseJoint = this.mouseJoint;
    ...
    mouseUp(evt) {
        var mouseJoint = this.mouseJoint, world = this.pLayer.world;
        if (mouseJoint) {
